...
import { getPedidos, getAllPedidos } from "../services/pedido.service";

const statusColors = {
  COTIZADO: { bg: "#E3F2FD", color: "#1976D2" },
  "EN PROCESO": { bg: "#FFF3E0", color: "#FF9800" },
  COMPLETADO: { bg: "#E8F5E9", color: "#4CAF50" },
  CANCELADO: { bg: "#FFEBEE", color: "#F44336" }
};

const OrdersTable = ({ isAdmin = false }) => {
  ...
  const loadData = async () => {
    try {
      const data = isAdmin ? await getAllPedidos() : await getPedidos();
      const mapped = data.map((item) => ({
        id: item.id,
        cliente: item.cliente?.replace(' null', '').trim() || '',
  ...
  const handleVer = () => {
    navigate(`/nueva-cotizacion/${selectedRow.id}`);
    handleCloseMenu();
  };
  ...
  return (
    <TableContainer>
      <Table>
        <TableHead>
          <TableRow>
            ...
            <TableCell sx={{ color: "#999", fontWeight: 500 }}>Fecha</TableCell>
          ...
          {rows.length === 0 ? (
            <TableRow>
              <TableCell
                colSpan={6}
                align="center"
                sx={{ py: 6, color: "#999", fontSize: "14px" }}
              >
              ...
              return (
                <TableRow key={row.id}>
                  ...
                  <TableCell>
                    ...
                    <div style={{ color: "#999", fontSize: "12px" }}>{row.hora}</div>
                  </TableCell>
      ...
      >
        <MenuItem onClick={handleVer}>Ver</MenuItem>
